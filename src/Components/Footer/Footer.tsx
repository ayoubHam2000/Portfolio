...
const Footer = () => {
  ...
  return (
    <footer className='pt-20 sm:pt-30 pb-8 mt-20 border-t-2 border-color-primary mx-36 text-2xl'>
      <div className='flex flex-col justify-center items-center gap-3'>
        ...
        <div className='flex gap-20 roboto_font' >
          <div> {profileService.phoneNumber} </div>
          <div> {profileService.email} </div>
          <div> {profileService.address.country} </div>
        </div>
        <div className='mt-16 sm:mt-28'> 2023 Typescript React & Tailwind CSS Portfolio {profileService.fullName} </div>
      </div>
    </footer>
  )
}
